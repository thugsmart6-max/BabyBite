...
import type { DailyPlan, GeneratedMealPlan, MealSlot, NutritionGoal } from "@/types/babybite";
import { GlassCard } from "@/components/babybite/ui";
import { MEAL_SLOT_LABELS } from "@/types/babybite";
        ...
        <tbody>
          {days.map((day) => (
            <tr key={day.date}>
              ...
              <td>
                {mealOf(day, "morningSnack")} · {mealOf(day, "eveningSnack")}
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {day.meals.map((meal, i) => (
        ...
        >
          ...
          <div className="min-w-0">
            ...
            {meal.minutes ? (
              <p className="text-xs text-muted-foreground">{meal.minutes} minutes</p>
            ) : null}
            {meal.pantry?.length ? (
              <p className="text-xs text-muted-foreground">From the box: {meal.pantry.join(" · ")}</p>
            ) : null}
            {meal.portionNote && (
              <p className="text-xs text-accent/80 mt-1">{meal.portionNote}</p>
            )}
            {meal.swaps?.length ? (
              <div className="mt-3 border-l-2 border-ink pl-2">
                <p className="label-caps text-accent mb-1">If they refuse this</p>
                {meal.swaps.map((swap) => (
                  <p key={swap.name} className="text-xs text-muted-foreground break-words mb-1">
                    {swap.name} — {swap.why}
                  </p>
                ))}
              </div>
