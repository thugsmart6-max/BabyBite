...
export type ChildGenderChoice = "male" | "female";

export function asChildGenderChoice(gender?: string | null): ChildGenderChoice {
...
export type CookTime = "ten-min" | "normal";
export type KitchenBudget = "tight" | "normal";
...
export const DEFAULT_RICE_HABIT: RiceHabit = "eats-rice";
export const DEFAULT_TIFFIN_NEED: TiffinNeed = "home-only";

export function kitchenFacts(profile: {
  cookTime?: CookTime;
  ...
  return {
    cookTime: profile.cookTime ?? DEFAULT_COOK_TIME,
    kitchenBudget: profile.kitchenBudget ?? DEFAULT_KITCHEN_BUDGET,
    riceHabit: profile.riceHabit ?? DEFAULT_RICE_HABIT,
    tiffinNeed: profile.tiffinNeed ?? DEFAULT_TIFFIN_NEED,
  };
}
...
export interface BabyBiteChildProfile {
  id: string;
  name: string;
  ageYears: number;
  gender: ChildGender;
...
export interface KitchenLists {
  ...
  tenMin: MealEntry[];
  budget: MealEntry[];
  schoolLunch: MealEntry[];
  kidsFavourite: MealEntry[];
  riceFree: MealEntry[];
  homemadeSnacks: MealEntry[];
}
